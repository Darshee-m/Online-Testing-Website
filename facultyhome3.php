...
if($_SESSION["usertype"] != "faculty") {
	...
	echo "<script type='text/javascript'>alert('You have not selected subject yet!'); 
...
<html>
	<head>
		...
		<?php
			...
			//get long name of sub
			$r1 = "select * from subject where short = '$subject_short' && branch = '$branch' && semester = '$semester' ";
			$r2 = mysqli_query($con, $r1);
			$r3 = mysqli_fetch_array($r2);
			$subject_name = $r3['name'];
			
			//set name of sub in subselected
			$r = "update temp set subject = '$subject_name' where email = '$email' ";
			mysqli_query($con, $r);
			$q = "update temp set short = '$subject_short' where email = '$email' ";
			mysqli_query($con, $q);
			?>
	</head>	
...
<body style="background:linear-gradient(#8ACBFA,white 100%); height:100%">
	...
	<?php
			$q1 = "select * from faculty where email = '$email' && subject = '$subject_short' && branch = '$branch' ";
			$q2 = mysqli_query($con, $q1);
		?>
	...
	<br><h1 style="text-align:center;">Welcome <?php //echo $_SESSION["username"];?></h1><br>
		...
		<form method="POST" action="facultyvalidation.php">	
			Select division :-
			<select name="division" id="division2" class="selectdivision2">
				<?php while($q3 = mysqli_fetch_array($q2)) { ?>
				<option value="<?php echo $q3['division']; ?>"><?php echo $q3['division']; ?></option>
				<?php } ?>
			</select><br><br>
